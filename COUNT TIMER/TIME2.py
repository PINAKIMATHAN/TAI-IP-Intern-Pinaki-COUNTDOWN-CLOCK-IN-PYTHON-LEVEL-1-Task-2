...
class Application(tk.Frame): 
    ...
    def build_interface(self):
        self.time_entry = tk.Entry(self, font=('Arial', 20, "bold"))
        self.time_entry.grid(row=0, column=1)

        self.clock = tk.Label(self, text="00:00:00", font=('Arial', 50, "bold"))
        self.clock.grid(row=1, column=1, stick="S")

        self.time_label = tk.Label(self, text="hour   min   sec", font=('Arial', 20, "bold"), width=15)
        self.time_label.grid(row=2, column=1, sticky="N")

        self.power_button = tk.Button(self, text="Start",font=('Arial', 20, "bold"), bg="blue", command=lambda: self.start())
        self.power_button.grid(row=3, column=0, sticky="NE")

        self.reset_button = tk.Button(self, text="Reset", font=('Arial', 20, "bold"), bg="yellow", command=lambda: self.reset())
        self.reset_button.grid(row=3, column=1, sticky="NW")

        self.quit_button = tk.Button(self, text="Quit", font=('Arial', 20, "bold"), bg="red", command=lambda: self.quit())
        self.quit_button.grid(row=3, column=3, sticky="NE")

        self.pause_button = tk.Button(self, text="Pause", font=('Arial', 20, "bold"), bg="green", command=lambda: self.pause())
